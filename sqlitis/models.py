...
class Select(Base):
    ...
    def render(self):
        ...
        if not self._cols:
            return 'select()'
        return 'select([%s])' % ", ".join("'%s'" % c for c in self._cols)
...
class Field(Base):
    ...
    def render(self):
        if self.literal:
            return self.name
        return convert_column(self.name)
...
class Join(Base):
    ...
    def Field(self, name, literal=False):
        self._tables[-1].clause.Field(name, literal)
        return self
...
class Comparison(Base):
    ...
    def Field(self, name, literal=False):
        if not self.left:
            self.left = Field(name, literal)
        elif not self.right:
            self.right = Field(name, literal)
        else:
